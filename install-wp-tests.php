...
if ( ! is_file( $wp_tests_dir . '/wp-tests-config.php' ) ) {
	...
	$contents = str_replace(
		array( 'youremptytestdbnamehere', 'yourusernamehere', 'yourpasswordhere', "dirname( __FILE__ ) . '/src/'" ),
		array( $db, $username, $password, "dirname( dirname( __FILE__ ) ) . '/wordpress/'" ),
		$contents
	);
...
function _manually_load_plugin() {
	require dirname( dirname( __FILE__ ) ) . \'/thrive-ultimatum.php\';
}
...
';
file_put_contents( __DIR__ . '/../bootstrap.php', $bootstrap );
...
';
file_put_contents( __DIR__ . '/../../phpunit.xml.dist', $phpunit );
...
';
file_put_contents( __DIR__ . '/../test-sample.php', $sample );
...
$driver->query( "CREATE DATABASE `{$db}`" );
